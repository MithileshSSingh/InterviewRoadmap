...
export default function LearningPhases({ phases = [], roadmapId, onTopicToggle }) {
    ...
    <section style={{ marginBottom: "1.25rem" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {phases.map((phase, idx) => {
          const completedCount = phase.topics?.filter((t) => t.completed).length ?? 0;
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "0.8rem", color: "var(--text-secondary)" }}>
                    {phase.durationWeeks} weeks · {total} topics · {pct}% complete
                  </div>
                </div>

                {/* Phase progress mini-bar */}
                <div style={{ width: 60, display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.25rem" }}>
                  <span style={{ fontSize: "0.75rem", fontWeight: 700, color: pct === 100 ? "var(--accent-blue)" : "var(--text-secondary)" }}>
                    {pct}%
                  </span>
